...
impl Ticket for PresenceTicket {
    const KIND: &'static str = "cofracture-presence";
...
impl PresenceNode {
    pub async fn spawn(secret_key: Option<SecretKey>) -> Result<Self> {
        ...
        info!("endpoint bound, id: {endpoint_id:#?}");
...
impl PresenceSender {
    pub async fn broadcast_presence(
        ...
        {
            let mut state = self.presence_state.lock().expect("poisoned");
            state.bounds = Some(bounds.clone());
...
pub enum Event {
    ...
    Joined { neighbors: Vec<EndpointId> },
    #[serde(rename_all = "camelCase")]
    Presence {
        from: EndpointId,
        bounds: PresenceBounds,
        ...
        let converted = match event {
            ...
            GossipEvent::Received(message) => {
                ...
                match message.message {
                    Message::Presence { bounds, color } => Self::Presence {
                        from: message.from,
                        bounds,
                        color,
